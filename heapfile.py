...
class MinHeap:
    ...
    def insert(self, value):
        ...
        while (current > 0 and 
               self.parameter(self.heap[current]) < self.parameter(self.heap[self._parent(current)])):
            self._swap(current, self._parent(current))
            current = self._parent(current)
    ...
    def _sink_down(self, index):
        ...
        while True:
            ...
            if (left < len(self.heap) and 
                self.parameter(self.heap[left]) < self.parameter(self.heap[min_index])):
                min_index = left
                
            if (right < len(self.heap) and 
                self.parameter(self.heap[right]) < self.parameter(self.heap[min_index])):
                min_index = right
